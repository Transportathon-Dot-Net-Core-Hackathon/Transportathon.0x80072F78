...
namespace Transportathon._0x80072F78.API.Controllers
{
    public class AddressController : CustomBaseController
    {
        ...
        [HttpGet]
        public async Task<ActionResult<CustomResponse<List<AddressDTO>>>> GetOutputAddresses()
        {
            return CreateActionResultInstance(await _addressService.GetOutputAddressesAsync());

        }

        [HttpGet]
        public async Task<ActionResult<CustomResponse<List<AddressDTO>>>> GetDestinationAddresses()
        {
            return CreateActionResultInstance(await _addressService.GetDestinationAddressesAsync());
